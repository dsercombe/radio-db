...
interface PnLTradeChartProps {
  strategyName?: string;
  limit?: number;
}

export function PnLTradeChart({ strategyName, limit = 100 }: PnLTradeChartProps): JSX.Element {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : "Fehler beim Laden der PnL/Trade-Timeline");
    } finally {
      setLoading(false);
    }
  }, [strategyName, limit]);
  ...
  if (loading && timeline.length === 0) {
    return <div style={{ padding: "20px", textAlign: "center" }}>Lade PnL/Trade-Timeline...</div>;
  }
  ...
  return (
    <div style={{ padding: "20px" }}>
      <h3>PnL/Trade Verlauf über Zeit</h3>
      <ResponsiveContainer width="100%" height={400}>
        <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
          <CartesianGrid strokeDasharray="3 3" />
          <XAxis dataKey="timestamp" angle={-45} textAnchor="end" height={100} />
          <YAxis yAxisId="left" label={{ value: "PnL/Trade", angle: -90, position: "insideLeft" }} />
