...
def create_session(request: BrowserSessionCreateRequest) -> BrowserSessionStateResponse:
    try:
...
def navigate(session_id: str, request: BrowserSessionNavigateRequest) -> BrowserSessionStateResponse:
    try:
...
def action(session_id: str, request: BrowserSessionActionRequest) -> BrowserSessionStateResponse:
    try:
...
def snapshot(session_id: str) -> BrowserSessionStateResponse:
    try:
        return _serialize(snapshot_browser_session(session_id))
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="browser_session_not_found") from exc
